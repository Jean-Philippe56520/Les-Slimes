...
def test_unauthorized_actor_command_is_rejected_without_mutation(tmp_path):
    ...
    result = CanonicalWorldWorker(repo, holder_id="worker-auth").run_until(start)
...
def test_granting_permission_enables_actor_command(tmp_path):
    repo = build_repo(tmp_path)
    start = datetime(2026, 9, 17, 12, 0, tzinfo=UTC)
    CanonicalRuntime(repo).ensure_initialized(start)
    storage = RuntimeStorage(repo)
...
def test_inactive_actor_is_rejected_even_with_permission(tmp_path):
    repo = build_repo(tmp_path)
    start = datetime(2026, 9, 17, 12, 0, tzinfo=UTC)
    CanonicalRuntime(repo).ensure_initialized(start)
    storage = RuntimeStorage(repo)
    storage.set_actor_permissions("order", [ActorPermission.DEPOSIT_FOOD])
    storage.set_actor_active("order", False)
